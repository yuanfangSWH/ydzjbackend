  ...
  <div class="shopSet">
    ...
    <div class="shanxdiv">
      <el-input
        ...
        placeholder="操作内容"
        v-model="searchOption.log_content"
        clearable
      ></el-input>
      <el-input
        style="width: 350px; margin-right: 10px"
    ...
    >
      ...
      <el-table-column
        ...
        sortable="true"
      />
      <el-table-column prop="log_ip" label="IP" width="200" sortable="true" />
      <el-table-column
        prop="log_time"
        label="操作时间"
        width="200"
        sortable="true"
      />
      <el-table-column prop="log_content" label="操作内容" width="350" />
      <el-table-column prop="log_url" label="操作接口" width="350">
        <template slot-scope="scope">
          {{ scope.row.log_url ? decodeURIComponent(scope.row.log_url) : "" }}
        </template>
      </el-table-column>

      <!-- <el-table-column prop="content" label="错误内容" width="350" />
      <el-table-column prop="file" label="错误文件" width="250" />
      <el-table-column prop="line" label="错误文件行数" width="120" /> -->
    ...
    <el-dialog title="详情信息" :visible.sync="detailsDialog" width="50%">
      <el-form :v-model="detailsData" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="管理员：">
          {{ detailsData.admin_name }} (ID：{{ detailsData.admin_id }})
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    iniPageParam() {
      this.searchOption = {
        log_content: this.$route.query.log_content,
    ...
    getTableList() {
      ...
      loading: true;
      this.api.system.AdminLog(data).then((res) => {
        loading: false;
    ...
    detailsDelDialog(data) {
      ...
      this.detailsData.log_url = this.detailsData.log_url
        ? decodeURIComponent(this.detailsData.log_url)
        : "";
