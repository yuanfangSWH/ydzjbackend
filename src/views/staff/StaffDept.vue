...
export default {
  ...
  methods: {
    ...
    insertItemtk() {
      ...
      this.dept_status_ui = true;
    },
    /* 新增 */
    insertItem() {
      var data = this.addItem;
      if (this.dept_status_ui) {
        data.dept_status = 1;
      } else {
        data.dept_status = 0;
      }
      this.api.staff.deptInsert(data).then((res) => {
    ...
    openUpdateDialog(index, row) {
      ...
      this.currenItem = row;
      if (this.currenItem.dept_status == 1) {
        this.dept_status_ui = true;
      } else {
        this.dept_status_ui = false;
      }
    },
    /* 修改 */
    multiUpdate() {
      ...
      this.api.staff.deptUpdate(data).then((res) => {
        this.getTableList();
    ...
    deleteItem(type) {
      ...
      this.api.staff.deptDelete(data).then((res) => {
        this.getTableList();
