...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        //   {
        //     type: "array",
        //     required: this.ruleForm.list_shape != 3,
        //     message: "请选择图片/视频",
        //     trigger: "blur",
        //   },
        // ],
  ...
  created() {
    ...
    this.ruleForm.news_id = Number(this.$route.query.news_id);
    this.getNewsList();
    this.getGoodsClass();
  },
  methods: {
    ...
    getNewsList() {
      this.api.zhixun
        .getNews({ news_id: this.ruleForm.news_id })
        .then((res) => {
          //标题
          this.ruleForm.title = res.data.data[0].title;
          //概要
          this.ruleForm.summary = res.data.data[0].summary;
          //富文本内容
          this.ruleForm.content = res.data.data[0].content;
          //文章类型
          this.ruleForm.list_shape = res.data.data[0].list_shape;
          if (res.data.data[0].category) {
            var feng = res.data.data[0].category;
            for (var i of feng) {
              //分类ID
              this.ruleForm.mid.push(i.mid);
            }
          }
          //原创
          this.ruleForm.is_original = res.data.data[0].is_original;
          //允许评论 ：1是 0否
          this.ruleForm.allow_comment = res.data.data[0].allow_comment;
          //置顶状态：0不置顶 1置顶
          this.ruleForm.stick = res.data.data[0].stick;
          //状态：1正常 2禁用
          this.ruleForm.status = res.data.data[0].status;
          //封面
          this.ruleForm.covers =
            String(res.data.data[0].images) != "null"
          ...
          if (res.data.data[0].video) {
            //视频
            this.videoIMG =
              String(res.data.data[0].video) != "null"
                ? res.data.data[0].video.path
                : "";
            //上传视频ID
            this.ruleForm.video_id = res.data.data[0].video.video_id;
          }
          //省份ID
          this.ruleForm.province_id = res.data.data[0].province_id;
          //城市ID
          this.ruleForm.city_id = res.data.data[0].city_id;
          //区县ID
          this.ruleForm.area_id = res.data.data[0].area_id;
          //推送时间 2020-01-02 15:04
          this.ruleForm.push_time = res.data.data[0].push_time;
          //关联商家
          if (res.data.data[0].shop != undefined) {
            var sjia = res.data.data[0].shop;
            this.list = sjia.map((item) => {
              return {
            ...
            //关联商家ID数组
            this.ruleForm.shop_id = sjia.map((item) => {
              return item.shop_id;
            });
          }
          //地区选择器组件参数
    ...
    // 获取子组件返回的图片数据
    parentImg(url, localPath, n, file) {
    ...
    tijiao() {
      ...
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          ...
          //return;
          this.api.zhixun.newsUpdate(data).then((res) => {
            this.$message({
              message: "添加成功",
              type: "success",
            });
