  ...
  <div class="shopSet">
    <div class="box2">
      ...
      <div class="add-box2">
        <div class="btn-nav" v-if="isPass">
          <el-button v-if="status == 3" type="danger" @click="batcDdelFun(1, 0)"
            >通过</el-button
          >
          <el-button v-if="status == 3" type="danger" @click="batcDdelFun(2, 0)"
            >不通过</el-button
          >
      ...
      >
        ...
        <el-table-column prop="content" label="评论内容" width="320">
          <template slot-scope="scope">
            <el-tooltip
              class="item"
              effect="dark"
        ...
        <el-table-column prop="title" label="评论资讯" width="300">
          <template slot-scope="scope">
            <el-tooltip
              class="item"
              effect="dark"
            ...
            >
              <div @click="toNews(scope.row.news_id)">
        ...
        <el-table-column prop="reply_count" label="回复" width="100">
          <template slot-scope="scope">
            <div @click="toReplyList(scope.row)" style="cursor: pointer">
            ...
            <div v-if="isPass">
              <div v-if="scope.row.status == 3">
                ...
                <el-button
                  ...
                  size="mini"
                  type="text"
                  @click="openpassFun(scope.row.comment_id, 1, 1)"
                  >通过</el-button
                >
                <el-button
                  class="t02"
                  size="mini"
                  type="text"
                  @click="openpassFun(scope.row.comment_id, 2, 1)"
                  >不通过</el-button
                >
              </div>
              <div v-if="scope.row.status == 1" style="font-size: 14px">
                <!-- <el-button class="t01"
                           size="mini"
                           style="color:#409eff"
                           type="text"
                ...
                <el-button size="mini" type="text" disabled>已通过</el-button>
              </div>
            </div>
            <div v-else>
              <!-- <el-button class="t01"
    ...
    <el-dialog title="动态详情" :visible.sync="showDetail" width="50%">
      <template v-if="showDetail">
        <comment-Info :thread_id="infoThread_id" @handleClose="handleClose">
        </comment-Info>
      </template>
    </el-dialog>
    <!-- 单条 -->
...
export default {
  ...
  created() {
    if (sessionStorage.getItem("newsCommetPage")) {
      this.curPage = sessionStorage.getItem("newsCommetPage");
    }
    // 返回上一页
    ...
    if (this.$route.query.root_id) {
      this.status = null;
      this.root_id = this.$route.query.root_id;
    }
  ...
  methods: {
    ...
    getTableList() {
      ...
      this.api.community.newsCommentList(data).then((res) => {
    ...
    batcDdelFun(n, i) {
      ...
      this.handleStatus = n;
      this.isBatch = i;
      this.titleText = n == 1 ? "通过" : "不通过";
      if (this.multipleSelection != "") {
        this.dialogHandleMore = true;
      } else {
        this.$message({
          message: "请先勾选需要" + this.titleText + "的数据",
          type: "warning",
        });
      }
    },
    ...
    pageChange(page) {
      this.curPage = page;
      this.getTableList();
      sessionStorage.setItem("newsCommetPage", page);
    },
    openpassFun(id, n, i) {
      this.titleText = n == 1 ? "通过" : "不通过";
      this.comment_id = id;
    ...
    passFun() {
      ...
      if (this.isBatch == 0) {
        //批量操作
        data = {
          comment_id: this.multipleSelection,
      ...
      this.api.community.newsCommentHanding(data).then((res) => {
        this.$message({
          message: "" + res.data.msg,
          type: "success",
        });
    ...
    toReplyList(row) {
      let routeData = this.$router.resolve({
        path: "/news/comment?root_id=" + row.comment_id+"&page=1",
      });
      window.open(routeData.href, "_blank");
    },
    toNews(news_id) {
      this.$router.push("/news/content?news_id=" + news_id + "&from=true");
    },
  },
};
</script>
