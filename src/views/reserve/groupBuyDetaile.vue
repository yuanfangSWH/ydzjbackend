...
export default {
  ...
  created() {
    ...
    if (this.group_buy_id) {
      this.groupBuyDetaile();
    }
  },
  methods: {
    //预定订单详情
    groupBuyDetaile() {
      this.loading = true;
      let data = { group_buy_id: this.group_buy_id };
      this.api.reserve.getGroupBuyDetail(data).then((res) => {
        this.loading = false;
        this.groupBuyOrderData = res.data.data;
        // this.placeArr.push(this.groupBuyOrderData.reserve)
        // this.groupBuyOrderData.progress.forEach((item, index) => {
        //     if(item.active == 1){
        //         this.stepNum++
        //     }
        // })
