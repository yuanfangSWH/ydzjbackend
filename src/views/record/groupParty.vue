<template>
  <div class="shopSet">
    <div class="filterBox">
      <div class="g-title1">组局数据</div>
      <groupparty-item @filtrateFun='prantFun'></groupparty-item>
    </div>
    ...
    <div class="navBox-2">
      <el-table
        ...
        <el-table-column
          ...
          width="">
          <template slot-scope="scope">
            <span v-if="scope.row.is_arrive == 0">未到场</span>
            <span v-if="scope.row.is_arrive == 1">已到场</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="create_time"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.status == 2">截止报名</span>
            <span v-if="scope.row.status == 3">活动中</span>
            <span v-if="scope.row.status == 4">已完成</span>
            <span v-if="scope.row.status == 5">发起者取消</span>
            <span v-if="scope.row.status == 6">超时完成</span>
            <span v-if="scope.row.status == 7">超时取消</span>
            <span v-if="scope.row.status == 8">违规</span>
          </template>
        </el-table-column>
      </el-table>
...
import grouppartyItem from "./components/grouppartyItem"
export default {
  name: 'reportGroupparty',
  components: {
    grouppartyItem
  },
  data() {
  ...
  methods: {
    getData() {
      ...
      this.api.record.groupPartyList(data).then((res) => {
    ...
    getChart(func) {
      ...
      this.api.record.groupPartyCharts(data).then((res) => {
        ...
        this.chartData.gift_val = this.chartData.gift_val.reverse()
    ...
    prantFun(shop_type, status, start, end, charts_type,is_arrive, n) {
      this.curPage = 1
    ...
    drawLine(){
      var _this = this
        ...
        myChart.setOption({
            ...
            legend: {
                  data: ['组局数', '报名人数', '礼物价值', '入局人数']
              },
            color:['#FFBE11','#B8B8B8', '#2183cf', '#058420'], 
            grid: {
                left: '3%',
                right: '4%',
                bottom: '3%',
