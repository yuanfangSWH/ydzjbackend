<template>
  <div class="shopSet">
    <div class="add-box">
      ...
      <div class="so-nav">
        <el-input
          ...
          clearable
        ></el-input>
        <el-button type="primary" @click="searchDatas" style="margin-left: 10px"
          >搜索</el-button
        >
      </div>
    </div>
    ...
    >
      ...
      <el-table-column prop="sn" label="编号" sortable="custom" />
        ...
        <el-col :span="12">
          ...
          >
            <el-form-item label="型号" prop="brand">
              <el-select
                v-model="currenPrinter.brand"
                placeholder="选择商家类型"
              >
                <el-option
                  v-for="(item, index) in printerBrandType"
                  :key="'brand_' + index"
                  :label="item"
                  :value="item"
                >
            ...
            <el-form-item label="是否启用">
              <el-checkbox v-model="currenPrinter.status"></el-checkbox>
            </el-form-item>
            <el-form-item>
              <el-button
                @click="
            ...
            <el-form-item class="printerImg"> </el-form-item>
          </el-form>
        </el-col>
        <el-col :span="12"
          ><div>打印例图：</div>
        ...
        <el-col :span="12">
          ...
          >
            ...
            <el-form-item label="自定义信息">
              ...
              <el-checkbox v-model="addPrinterData.custom_info_larger"
                >放大自定义信息</el-checkbox
              >
            </el-form-item>
            <el-form-item label="">
              <el-checkbox v-model="addPrinterData.show_qr"
                >是否显示二维码</el-checkbox
              >
            </el-form-item>
            <el-form-item label="">
              <el-checkbox v-model="addPrinterData.show_phone"
                >是否显示商家信息</el-checkbox
              >
            </el-form-item>
            <el-form-item label="是否启用">
...
export default {
  ...
  methods: {
    ...
    getPrinterBrandType() {
      ...
      this.api.shop.printerBrandType(datas).then(({ data }) => {
        this.printerBrandType = data.data.printer_brand_type;
      });
    },
    getPrinterList(page) {
    ...
    updatePrinter(formName) {
      ...
      this.currenPrinter.shop_id = Number(this.merchants.shop_id);
      this.currenPrinter.merchant_id = Number(this.merchants.merchant_id);
      this.currenPrinter.status = this.currenPrinter.status ? 1 : 0;
    ...
    insertPrinter() {
      var datas = {
        ...
        status: this.addPrinterData.status ? 1 : 0,
      };
    ...
    deletePrinter(type) {
      ...
      var id = type === "one" ? oneId : this.multipleSelection;
      console.info(id);
      const datas = {
        merchant_id: Number(this.merchants.merchant_id),
        shop_id: Number(this.merchants.shop_id),
    ...
    updateDialog(index, row) {
      ...
      this.currenPrinter = {
        ...
        custom_info_larger: row.custom_info_larger === 1,
        show_qr: row.show_qr === 1,
    ...
    selectPrinter(val) {
      ...
      val.forEach((item, index) => {
        ...
        this.multipleSelection.push(item.printer_id);
