<template>
  <div class="shopSet">
    <div class="pageTitle">“{{ userInfo.nickname }}”的优惠券</div>
    <div class="shanxdiv">
    ...
    >
      <el-table-column
        ...
        sortable="true"
      />
      <el-table-column prop="coupon_name" label="优惠券" />
      <el-table-column prop="source" label="价值">
      ...
      <el-table-column prop="source" label="来源">
        <template slot-scope="scope">
          {{ sourceType[scope.row.source - 1] }}
        </template>
      </el-table-column>
      <el-table-column prop="used_time" label="有效期" />
      <el-table-column prop="use_end_time" label="兑换时间" />
      <el-table-column prop="status" label="状态">
        <template slot-scope="scope">
          {{ sourceType[scope.row.source - 1] }}
        </template>
      </el-table-column>
      <el-table-column prop="order_no" label="使用券订单">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    iniPageParam() {
      ...
      this.searchOption = {
        user_id: this.$route.query.user_id
          ? Number(this.$route.query.user_id)
          : null,
    ...
    getTableList() {
      ...
      console.log(data);
      this.loading = true;
      this.api.user.UserCoupon(data).then((res) => {
        this.loading = false;
        this.tableListData = res.data.data;
        this.total = res.data.count ? res.data.count : 0;
        if (this.tableListData.length > 0) {
          this.userInfo = this.tableListData[0].user;
          this.userInfo.nickname = this.getAgent(this.userInfo.nickname);
        }
      });
    },
    /* 打开详情框 */
    ...
    pageChange(p) {
      if (!this.searchTime) {
        this.searchTime = ["", ""];
        this.searchOption.start = this.searchTime[0];
        this.searchOption.end = this.searchTime[1];
      }
