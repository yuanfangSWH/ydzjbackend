  ...
  <div class="shopSet">
    ...
    <div class="shanxdiv">
      <el-tabs v-model="activeName"
        ...
        <el-tab-pane label="性格"
                     name="character"></el-tab-pane>
        <el-tab-pane label="兴趣"
                     name="boring"></el-tab-pane>
        <el-tab-pane label="自我评价"
                     name="evaluation"></el-tab-pane>
      </el-tabs>
    </div>
    <el-table ref="multipNum"
      ...
      <el-table-column sortable="custom"
                       ...
                       label="添加时间"
                       width="250" />
      <el-table-column column-key="tag_status"
                       :filters="[
          { text: '禁用', value: 0 },
          { text: '启用', value: 1 },
        ]"
                       ...
                       label="状态">
        <template slot-scope="scope">
          <span v-if="scope.row.tag_status == 0">禁用</span>
          <span v-else-if="scope.row.tag_status == 1">启用</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="新增"
      ...
      <el-form :model="addItem"
        ...
        <el-form-item label="名称"
                      prop="tag_name">
          <el-input v-model="addItem.tag_name"
                    clearable />
        </el-form-item>
        <el-form-item label="排序"
                      prop="tag_sort">
          <el-input type="Number"
                    v-model="addItem.tag_sort"
                    clearable />
        </el-form-item>
        <el-form-item label="状态"
                      prop="tag_status">
          <el-select v-model="addItem.tag_status"
                     placeholder="请选择">
            <el-option key="0"
                       label="禁用"
                       :value="0"></el-option>
    ...
    <el-dialog :title="'修改-' + updateDialogTitle"
      ...
      <el-form :model="currenItem"
        ...
        <el-form-item label="名称"
                      prop="tag_name">
          <el-input v-model="currenItem.tag_name"
                    clearable />
        </el-form-item>
        <el-form-item type="Number"
                      label="排序"
                      prop="tag_sort">
          <el-input v-model="currenItem.tag_sort"
                    clearable />
        </el-form-item>
        <el-form-item label="状态"
                      prop="tag_status">
          <el-select v-model="currenItem.tag_status"
                     placeholder="请选择">
            <el-option key="0"
                       label="禁用"
                       :value="0"></el-option>
...
export default {
  ...
  data () {
    return {
      ...
      total: 1,
      rules: {
        tag_name: [{ required: true, message: "请填写名称", trigger: "blur" }],
      },
  ...
  methods: {
    ...
    percentF (row, index) {
      // 判断非数字类型
      if (
        typeof Number(row.tag_sort) === "number" &&
        isNaN(Number(row.tag_sort))
      ) {
        this.$message({
          message: "请输入纯数字",
          type: "warning",
      ...
      if (this.sortData != row.tag_sort && row.tag_sort) {
        this.currenItem = row;
        this.currenItem.tag_sort = Number(this.currenItem.tag_sort);
        this.api.user.BqUpdate(this.currenItem).then((res) => {
          if (res.data.code == 0) {
            this.sortingid = "";
            this.$message({
              message: "修改排序成功",
    ...
    filterTagTable (data) {
      if (data.tag_status != "") {
        this.tag_status = Number(data.tag_status[0]);
      } else {
        this.tag_status = "";
      }
      this.getTableList();
    ...
    handleClick () {
      if (this.activeName == "character") {
        this.tag_type = 1;
        this.addItem.tag_type = 1;
        this.getTableList();
      } else if (this.activeName == "boring") {
        this.tag_type = 2;
        this.addItem.tag_type = 2;
        this.getTableList();
      } else if (this.activeName == "evaluation") {
        this.tag_type = 3;
        this.addItem.tag_type = 3;
        this.getTableList();
      }
    },
    /* 获取列表内容 */
    getTableList () {
      ...
      this.api.user.TagList(data).then((res) => {
    ...
    insertItem () {
      ...
      this.api.user.TagInsert(data).then((res) => {
    ...
    openUpdateDialog (row) {
      ...
      this.updateDialogTitle = row.tag_name;
      this.currenItem = row;
    },
    /* 修改保存 */
    multiUpdate () {
      ...
      this.api.user.TagUpdate(data).then((res) => {
        this.$message({
          message: "修改成功",
          type: "success",
        });
    ...
    deleteItem (type) {
      ...
      this.api.user.TagDelete(data).then((res) => {
        this.$message({
          message: "删除成功",
          type: "success",
        });
