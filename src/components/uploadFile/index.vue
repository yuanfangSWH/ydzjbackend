<template>
  <div class="uploadFiles">
    ...
    <!-- 超出数量的弹窗 -->
    <el-dialog title="提示"
               :visible.sync="chaochudialogVisible"
               width="30%">
      <span>{{ tishi }}</span>
      <span slot="footer"
            class="dialog-footer">
        ...
        <el-button type="primary"
                   @click="chaochudialogVisible = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
// import ConsolerVue from '@/views/mainConsole/Consoler.vue'
...
export default {
  ...
  methods: {
    ...
    uploadImg (option) {
      ...
      if (this.isSvg) {
        this.fileLists.forEach(function (file) {
          ...
          upData.append("root", "svg");
        });
      } else {
        this.fileLists.forEach(function (file) {
          // 因为要上传多个文件，所以需要遍历
          upData.append("file[]", file, file.name);
          upData.append("root", thisRoot);
        });
      }
      this.$ajax.defaults.headers.common["AccessToken"] = getToken();
      this.$ajax.post("/api/file/uploads", upData, this.header).then((res) => {
        ...
        if (this.limit == 1) {
          if (this.isSvg) {
            this.$emit("childrenFun", this.svgFile.raw, 1);
          } else {
    ...
    async handleUploadSTS (option) {
      ...
      try {
        this.clentSts = await this.$store
          .dispatch("shop/getFileSts")
          .then((res) => {
    ...
    beforeAvatarUpload (file) {
      console.log("55556666", file.type);
      if (this.isSvg) {
        if (file.type.indexOf("svg") !== -1) {
          return file;
        } else {
          this.$message.error("只能上传SVG图");
          return false;
        }
      } else {
